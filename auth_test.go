...
/dictionary/fran%C3%A7ais/pr%c3%a9f%c3%a8re`},
		[2]string{`GET /photos/puppy.jpg?AWSAccessKeyId=AKIAIOSFODNN7EXAMPLE&
...
/johnsmith/photos/puppy.jpg`},
	...
	for i, row := range table {
		row = [2]string{stripspaces(row[0], false), stripspaces(row[1], true)}
		r, err = http.ReadRequest(bufio.NewReader(strings.NewReader(row[0])))
		if err != nil {
			t.Fatalf("%d: bad request: %s", i, err)
			break
		}
		got = getBytesToSign(r, "s3.amazonaws.com")
		if string(got) != row[1] {
			t.Errorf("%d. got {{{%s}}}\n awaited [[[%s]]]", i, string(got), row[1])
			t.Fail()
		}
	}
}
